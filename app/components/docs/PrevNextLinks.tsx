import clsx from "clsx";
import { Link, useLocation } from "react-router";

import { navigation } from "~/app/lib/docs/navigation.ts";

function ArrowIcon(props: React.ComponentPropsWithoutRef<"svg">) {
	return (
		<svg viewBox="0 0 16 16" aria-hidden="true" {...props}>
			<path d="m9.182 13.423-1.17-1.16 3.505-3.505H3V7.065h8.517l-3.506-3.5L9.181 2.4l5.512 5.511-5.511 5.512Z" />
		</svg>
	);
}

function PageLink({
	title,
	href,
	dir = "next",
	...props
}: Omit<React.ComponentPropsWithoutRef<"div">, "dir" | "title"> & {
	title: string;
	href: string;
	dir?: "previous" | "next";
}) {
	return (
		<div {...props}>
			<dt className="font-display text-sm font-medium text-foreground">
				{dir === "next" ? "Next" : "Previous"}
			</dt>
			<dd className="mt-1">
				<Link
					to={href}
					className={clsx(
						"flex items-center gap-x-1 text-base font-semibold text-foreground-muted hover:text-foreground",
						dir === "previous" && "flex-row-reverse",
					)}
				>
					{title}
					<ArrowIcon
						className={clsx("h-4 w-4 flex-none fill-current", dir === "previous" && "-scale-x-100")}
					/>
				</Link>
			</dd>
		</div>
	);
}

/**
 * Previous/next page links at the foot of a doc, derived from the flattened nav
 * order and the current path. Renders nothing at the ends of the sequence.
 * Ported from the template; usePathname -> useLocation, next/link -> RR Link.
 */
export function PrevNextLinks() {
	const pathname = useLocation().pathname;
	const allLinks = navigation.flatMap((section) => section.links);
	const linkIndex = allLinks.findIndex((link) => link.href === pathname);
	const previousPage = linkIndex > -1 ? allLinks[linkIndex - 1] : null;
	const nextPage = linkIndex > -1 ? allLinks[linkIndex + 1] : null;

	if (!nextPage && !previousPage) {
		return null;
	}

	return (
		<dl className="mt-12 flex border-t border-border pt-6">
			{previousPage && <PageLink dir="previous" {...previousPage} />}
			{nextPage && <PageLink className="ml-auto text-right" {...nextPage} />}
		</dl>
	);
}
