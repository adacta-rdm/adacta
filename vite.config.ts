...
export default defineConfig(({ mode }) => ({
	...
	server: {
		...
		// We need to stick to this port due to CORS policy set on the repository server.
		strictPort: true,
	},
	plugins: [
		tsconfigPaths({ root: __dirname }),
		react({
			// Use JSX factory from emotion (mainly required to get support for the custom css
			// prop I guess)
	...
	resolve: {
		alias: {
			...
			// these modules, so we provide empty implementations here.
			// See lib/buildTimeConstants.ts for how these imports are used.
			...(mode === "test" || mode === "benchmark"
				? null
				: {
						"node:child_process": resolve("node_modules/@/nodejs"),
						"node:util": resolve("node_modules/@/nodejs"),
						"node:fs": resolve("node_modules/@/nodejs"),
				  }),
		},
	},
	define: {
		"process.env.REPO_SERVER_URL": JSON.stringify(process.env.REPO_SERVER_URL),
		"process.env.NODE_DEBUG": undefined,
		...buildTimeConstantsObject,
	},
}));

const emptyNodeJSImpl = Object.keys({ ...childProcess, ...fs, ...util })
	.map((key) => (key === "default" ? "export default {};" : `export const ${key} = () => {};`))
	.join("\n");

fs.mkdirSync(resolve("node_modules/@"), { recursive: true });
fs.writeFileSync(resolve("node_modules/@/nodejs.js"), emptyNodeJSImpl, "utf8");

// Taken from https://raw.githubusercontent.com/elastic/eui/master/src/components/icon/icon_map.ts
...
const iconImportStatements = Object.entries(typeToPathMap).map(([icon, path]) =>
	!(icon.startsWith("logo") || icon == "stats" || icon === "securityAnalyticsApp")
		...
		const _${icon} = undefined;`
);

const fileContents = `/* eslint-disable */
import { appendIconComponentCache } from "@elastic/eui/es/components/icon/icon";

${iconImportStatements.join("\n")}
